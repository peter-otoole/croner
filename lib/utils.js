...
exports.setLoggingLevel = function (levelParameter) {
  ...
  if (typeof levelParameter === 'undefined') {
    ...
    !Object.prototype.hasOwnProperty.call(exports.log, levelParameter)
  ) {
    throw new Error(
      `'${levelParameter}' is not a valid logging level - please choose one of ${Object.keys(
        exports.log
      ).toString()}`
    )
  } else {
  ...
  if (!date.isValid()) {
    ...
    if (!date.isValid()) {
      throw new InternalError(
        `Found invalid exif date '${timestamp}', unable to parse`
      )
    }
  }
