...
function readExifInformation(folderPath, files, callback) {
    ...
    let tasks = files.map( (file, possition) => {
        ...
        return function(callback) {
            ...
            try {
                ...
                log.trace( `Reading file '${file.fileName}' (${possition} of ${files.length})` )
                new ExifImage({ image: filePath }, function(error, exifData) {

                    if (error) {
                        error = new Error( `Failed reading file (${filePath}) Exif information - ${error.message}` )
                        log.error( `Tried reading file '${file.fileName}' (${possition} of ${files.length}) but got error '${error}'` )
                        return callback(error)
                    } else {
    ...
    async.parallelLimit(tasks, read_limit, (error, resutls) => {
