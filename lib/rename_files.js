        ...
        try {
            ...
            new ExifImage({ image: filePath }, function(error, exifData) {
                ...
                if (error) {
                    ...
                    log.error(`Tried reading exif data for file '${file.fileName}' (${position} of ${files.length}) but got error '${error}'`)
            ...
            error = new Error(`Failed reading file (${filePath}) Exif information - ${error.message}`)
            log.error(`Error when reading exif data for file '${file.fileName}' (${position} of ${files.length}) - '${error}'`)
            return callback(null, error)
        }
    })
...
function writeOutNewFileNames(folderPath, files, callback) {
    const tasks = files.map((file, position) => {
        return (callback) => {
            const filePath = path.join(folderPath, file.fileName)
            ...
            log.trace(`Renaming '${file.fileName}' to '${newFileName}' (${position} of ${files.length})`)
