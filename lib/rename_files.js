...
function renameFile (ignoreErrors, length, position, oldFileName, newFileName, callback) {
  fs.rename(oldFileName, newFileName, (error) => {
    if (error) {
      if (ignoreErrors) {
        log.warn(`Failed renaming '${oldFileName}' to '${newFileName}' (${position + 1} of ${length}) due to error - ${error.message}`)
        return callback()
      }
      return callback(new Error(`Failed to rename file '${oldFileName}' to '${newFileName}' - ${error.message}`))
    }
    return callback()
  })
}
