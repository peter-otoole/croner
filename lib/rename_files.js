...
module.exports = async function (
  ...
  log.info(
    `Start ordering files in '${targetFolder}' matching pattern '${pattern}' based on '${timestampType}'`
  )

  try {
    const fileNames = await getFileNames(targetFolder, pattern)
    const fileNamesInfo = await readAllFilesFsInformation(
      targetFolder,
      fileNames
    )
    const filteredFilesWithTimestamp = await readAllFilesTimestampInfo(
      ignoreErrors,
      timestampType,
      targetFolder,
      fileNamesInfo
    ...
    const count = await writeOutNewFileNames(
      ignoreErrors,
      targetFolder,
      newFileNames
    )
...
async function getFileNames(directory, pattern) {
  ...
  if (files.length === 0) {
    return Promise.reject(
      new Error('No files found matching the glob pattern'),
      null
    )
  }
...
async function readAllFilesFsInformation(targetFolder, fileNames) {
  const tasks = fileNames.map((fileName) => (callback) => {
    ...
    fs.stat(filePath, (error, stats) => {
      if (error) {
        return callback(
          new Error(
            `Failed reading file (${filePath}) information - ${error.message}`
          )
        )
      }
  ...
  return util.promisify(async.parallelLimit)(tasks, ReadLimit)
}
 ...
 */
async function readAllFilesTimestampInfo(
  ignoreErrors,
  timestampType,
  targetFolder,
  files
  ...
  if (timestampType !== 'exif') {
    log.debug(
      `Skipping loading of EXIF information because timestamp is set to '${timestampType}'`
    )
    files.forEach((file) => {
      file.timestamp = moment(file.stats[timestampType])
    })
  ...
  const tasks = files.map((file, position) => (callback) => {
    ...
    log.trace(
      `Reading EXIF data of file '${file.name}' (${position + 1} of ${
        files.length
      })`
    )
    const exif = new ExifImage({ image: filePath }, (error, exifData) => {
      if (error) {
        error = new Error(
          `Failed reading file (${filePath}) EXIF information - ${error.message}`
        )
        log.warn(
          `Failed reading EXIF data for file '${file.name}' (${
            position + 1
          } of ${files.length}) - '${error.message}'`
        )
      ...
      if (!exifData || !exifData.exif || !exifData.exif.DateTimeOriginal) {
        error = new Error(
          `Failed reading file (${filePath}) EXIF information; timestamp was null`
        )
        log.warn(
          `Failed reading EXIF data for file (${position + 1} of ${
            files.length
          }) - '${error.message}'`
        )
        return callback(null, { error })
    ...
    log.trace('EXIF is null at this point.', exif)
  })

  log.debug('Getting file EXIF information...')
  try {
    const results = await util.promisify(async.parallelLimit)(tasks, ReadLimit)
    ...
    if (ignoreErrors) {
      log.warn(
        `Failed getting EXIF information for ${failures.length} of ${results.length} files - '-i' flag was set, ignoring these errors`
      )
      return Promise.resolve(successes)
    }
    ...
    return Promise.reject(
      new Error(
        `Failed getting EXIF information for ${failures.length} of ${results.length} files` +
...
function generateNewFileNames(files) {
  ...
  files.forEach((f) => {
    ...
    f.newFileName = path.join(
      fileBasePath,
      f.timestamp.format('YYYYMMDD_HHmmss')
    )

    const filesWithSameNewName = newFiles.filter((file) => {
      return (
        path.parse(file.newFileName).name.slice(0, 15) ===
        path.parse(f.newFileName).name.slice(0, 15)
      )
    })
...
async function writeOutNewFileNames(ignoreErrors, targetFolder, files) {
  const giveTempName = files.map((file, position) => (callback) => {
    ...
    log.trace(
      `Renaming '${oldFilePath}' to '${tempFilePath}' (${position + 1} of ${
        files.length
      })`
    )
    renameFile(
  ...
  const givePermanentNames = files.map((file, position) => (callback) => {
    ...
    log.trace(
      `Renaming '${tempFilePath}' to '${newFilePath}' (${position + 1} of ${
        files.length
      })`
    )
    renameFile(
  ...
  try {
    await util.promisify(async.parallelLimit)(giveTempName, RenameLimit)
    await util.promisify(async.parallelLimit)(givePermanentNames, RenameLimit)
    return files.length
  } catch (error) {
...
function renameFile(
  ...
  fs.rename(oldFileName, newFileName, (error) => {
    if (error) {
      if (ignoreErrors) {
        log.warn(
          `Failed renaming '${oldFileName}' to '${newFileName}' (${
            position + 1
          } of ${length}) due to error - ${error.message}`
        )
        return callback()
      }
      return callback(
        new Error(
          `Failed to rename file '${oldFileName}' to '${newFileName}' - ${error.message}`
        )
      )
    }
    return callback()
