...
module.exports = async function (ignoreErrors, timestampType, targetFolder, pattern, callback) {
    ...
    try {
        ...
        writeOutNewFileNames(targetFolder, newFileNames)
...
async function readAllFilesFsInformation (targetFolder, filteredContents) {
    const tasks = filteredContents.map((fileName) => async (callback) => {
...
function generateNewFileNames (files) {
    ...
    files.forEach(f => {
        ...
        const filesWithSameNewName = newFiles.filter(file => file.newFileName == f.newFileName)

        if(filesWithSameNewName.length > 0) {
            f.newFileName += `_${filesWithSameNewName.length}`
        }

        f.newFileName += f.name.lastIndexOf(".")
    })
...
function writeOutNewFileNames (ignoreErrors, targetFolder, files) {
    const tasks = files.map((file, position) => async (callback) => {
        ...
        try {
            await util.promisify(fs.rename)(oldFilePath, newFilePath)
            return callback()
        } catch (error) {
            if(ignoreErrors){
