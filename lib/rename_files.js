...
function renameFiles(folderPath, files, callback) {
    ...
    let tasks = files.map((file, possition) => {
        ...
        let fileDateTime = file.cronerTimeStamp.format("YYYYMMDD_HHmmss_SSS")
        let newFileName = `${fileDateTime}${file.fileName.slice(file.fileName.lastIndexOf( "." ))}`

        let newFilePath = path.join(folderPath, newFileName)

        return function(callback) {

            log.trace(`Renaming '${file.fileName}' to '${newFilePath}' (${possition} of ${files.length})`)
            fs.rename(filePath, newFilePath, function(error) {

                if (error) {
                    error.messsage = `Failed to rename file "${filePath}" to "${newFilePath}" - ${error.message}`
                }
