...
function readAllFilesTimestampInfo (ignoreErrors, timestampType, targetFolder, files) {
    if (timestampType !== "exif") {
        ...
        files.forEach((file) => {
            let timestamp = currentFile.stats[timestampType]
            file.timestamp = moment(timestamp)
        })
    ...
    let tasks = files.map((file, position) => {
        ...
        try {
            ...
            let timestamp = currentFile.exif.exif.DateTimeOriginal
            file.timestamp = utils.parseExifDate(timestamp)
