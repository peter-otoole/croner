...
function temporarilyRenameFiles(folderPath, files, callback) {
    ...
    let tasks = files.map((file, possition) => {
        ...
        let filePath = path.join(folderPath, file.fileName)
        let tempName = `CR_TEMP_FILE_${utils.uuid()}${file.fileName.slice(file.fileName.lastIndexOf( "." ))}`
...
function renameFiles(folderPath, files, callback) {
    ...
    let tasks = files.map((file, possition) => {
        ...
        let fileTime = file.cronerTimeStamp.format("HHmmssSSS")
        let newFileName = `CR_${fileDate}_${fileTime}${file.fileName.slice(file.fileName.lastIndexOf( "." ))}`

        let nwFilePath = path.join(folderPath, newFileName)

        return function(callback) {

            log.trace( `Renaming '${file.fileName}' to '${nwFilePath}' (${possition} of ${files.length})` )
            fs.rename(filePath, nwFilePath, function(error) {

                if (error) {
                    error.messsage = `Failed to rename file "${filePath}" to "${nwFilePath}" - ${error.message}`
                }
