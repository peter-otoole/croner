...
function parseOriginalDateTime(files, callback) {
    ...
    files.forEach(file => {
        ...
        file.cronerTimeStamp = utils.parseExifDate(file.exif.exif.DateTimeOriginal)
    })
...
function orderArrayOnTimestamp(files, callback) {
    ...
    log.debug(`files before sorting`)
    files.forEach(file => {
        console.log(file.fileName + " \t\t- " + file.exif.exif.DateTimeOriginal + " - " + file.cronerTimeStamp)
    })
...
function renameFiles(folderPath, files, callback) {
    ...
    let tasks = files.map((file, possition) => {
        ...
        let newFileName = path.join(folderPath, `CR_${fileDate}_${fileOrder+1}.jpg`)
