...
exports.attachListener = function attachListener(toolName) {
  process.on('uncaughtException', (error) => {
    ...
    console.error(
      color.blackBG(
        `\n\nFailed running ${color.yellow(
          toolName
        )} due to exception - ${color.red(error.message)}`
      )
    )
    ...
    if (utils.isVerbose()) {
      ...
      console.error(
        "Run again with '-v' to get more information on this error."
      )
    }

    console.log(
      color.gray(
        'Please view the help to ensure you are using the tool correctly and contact the tool author for more information.'
      )
    )
