...
Examples:
  ...
  return yargs
    ...
    .option('f', {
      alias: 'folder',
      describe:
        'the folder location of the files to be sorted, defaults to current directory',
      type: 'string',
      demand: false
    })
    .option('p', {
      alias: 'pattern',
      describe:
        "glob filename pattern to match, defaults to '**/*.jpg'. NOTE: always case insensitive",
      type: 'string',
      demand: false
    })
    .option('i', {
    ...
    .option('v', {
      ...
      describe: `set logging level to ${Object.keys(
        utils.log
      ).toString()} (default to info, '' means debug)`,
      type: 'string',
      demand: false
    })
    .help('h')
